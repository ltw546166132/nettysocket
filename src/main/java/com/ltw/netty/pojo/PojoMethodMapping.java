package com.ltw.netty.pojo;

import com.ltw.netty.annotation.*;
import com.ltw.netty.exception.DeploymentException;
import com.ltw.netty.support.*;
...
public class PojoMethodMapping {
    ...
    public PojoMethodMapping(Class<?> pojoClazz, ApplicationContext context, AbstractBeanFactory beanFactory) throws DeploymentException {
        ...
        if(pojoClazz.getName().contains("$")){
           String[] pojoClazzArray = pojoClazz.getName().split("$");
           this.pojoClazzName = pojoClazzArray[0];
        }
        ...
        /*if (handshake != null && handshake.getDeclaringClass()!= pojoClazz.getSuperclass()) {
            if (isOverridenWithoutAnnotation(pojoClazzMethods, handshake, BeforeHandshake.class)) {
                handshake = null;
            }
        }
        if (open != null && open.getDeclaringClass()!= pojoClazz.getSuperclass()) {
            if (isOverridenWithoutAnnotation(pojoClazzMethods, open, OnOpen.class)) {
                open = null;
            }
        }
        if (close != null && close.getDeclaringClass()!= pojoClazz.getSuperclass()) {
            if (isOverridenWithoutAnnotation(pojoClazzMethods, close, OnClose.class)) {
                close = null;
            }
        }
        if (error != null && error.getDeclaringClass()!= pojoClazz.getSuperclass()) {
            if (isOverridenWithoutAnnotation(pojoClazzMethods, error, OnError.class)) {
                error = null;
            }
        }
        if (message != null && message.getDeclaringClass()!= pojoClazz.getSuperclass()) {
            if (isOverridenWithoutAnnotation(pojoClazzMethods, message, OnMessage.class)) {
                message = null;
            }
        }
        if (binary != null && binary.getDeclaringClass()!= pojoClazz.getSuperclass()) {
            if (isOverridenWithoutAnnotation(pojoClazzMethods, binary, OnBinary.class)) {
                binary = null;
            }
        }
        if (event != null && event.getDeclaringClass()!= pojoClazz.getSuperclass()) {
